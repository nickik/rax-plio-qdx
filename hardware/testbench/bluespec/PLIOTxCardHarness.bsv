...
typedef enum {
    ...
    CTxTurn, CTxControl, CTxDataLo, CTxDataHi, CTxArm, CTxExpose, CTxDone
...
module mkPLIOTxCardHarness(PLIOTxCardHarnessIfc);
    ...
    function BackplaneSample sampleImage(PlioIn x);
        ...
        b.control = BackplaneControl {
            space: pack(x.space),
            addressStrobe: x.addressStrobe,
            read: x.read,
            byteEnable: x.byteEnable,
            burstLen: pack(x.burst),
            dataStrobe: x.dataStrobe
        };
        b.ack = x.ack;
        b.err = x.err;
        b.selected = x.selected;
        b.grant = x.grant;
        return b;
    endfunction
    ...
    function Bool inHasData(PlioIn x);
        return x.adValid && x.parValid;
    endfunction
    ...
    function Bool outHasData(PlioOut x);
        return x.adValid && x.parValid;
    endfunction

    function Bool needsOutDrive(PlioOut x);
        return outHasControl(x) || outHasData(x);
    endfunction

    function QicPtiDrive rxSelect(PtiTokenKind kind);
        QicPtiDrive q = qicPtiDriveDefault();
        q.direction = PtiTxToQic;
        q.token = ptiToken(kind, 0, 0);
        return q;
    endfunction

    function QicPtiDrive txToken(PtiToken token);
        QicPtiDrive q = qicPtiDriveDefault();
        q.direction = PtiQicToTx;
        q.token = token;
        return q;
    endfunction
    ...
    function PtiControlImage outControl(PlioOut x);
        return PtiControlImage {
            ...
            burstLen: pack(x.burst),
            dataStrobe: x.dataStrobe,
            driveAdPar: outHasData(x),
            driveControl: outHasControl(x)
        };
    endfunction
        ...
        action
            ...
            if (reset) begin
                ...
                CRxTurn: begin
                    ...
                    PlioIn s = sampled;
                    s.reset = heldIn.reset;
                    s.selected = obs.sampleSelected;
                    s.grant = obs.sampleGrant;
                    s.ack = obs.sampleAck;
                    s.err = obs.sampleErr;
                    sampled <= s;
                    tx.advance(False, q, b);
                ...
                CRxControl: begin
                    ...
                    if (obs.rxValid) begin
                        ...
                        PlioIn s = sampled;
                        s.spaceValid = heldIn.spaceValid;
                        s.space = unpack(c.space);
                        s.addressStrobe = c.addressStrobe;
                        s.read = c.read;
                        s.byteEnable = c.byteEnable;
                        s.burst = unpack(c.burstLen);
                        s.dataStrobe = c.dataStrobe;
                        sampled <= s;
                    end
                    tx.advance(False, q, b);
                ...
                CRxDataLo: begin
                    ...
                    obs = tx.observeQic(False, q, b);
                    if (obs.rxValid) begin
                        rxLow <= ptiData(obs.rxToken);
                        rxLowPar <= ptiParity(obs.rxToken);
                    end
                    tx.advance(False, q, b);
                    state <= CRxDataHi;
                end
                ...
                CRxDataHi: begin
                    ...
                    if (obs.rxValid) begin
                        PlioIn s = sampled;
                        s.adValid = True;
                        s.ad = { ptiData(obs.rxToken), rxLow };
                        s.parValid = True;
                        s.parity = { ptiParity(obs.rxToken), rxLowPar };
                        sampled <= s;
                    end
                    tx.advance(False, q, b);
                    state <= CRxDone;
                ...
                // First idle slot turns the PTI direction toward PLIO-TX.
                CTxTurn: begin
                    q = txToken(ptiToken(PtiIdle, 0, 0));
                    tx.advance(False, q, b);
                ...
                CTxControl: begin
                    q = txToken(controlToken(outControl(heldOut)));
                    tx.advance(False, q, b);
                    state <= outHasData(heldOut) ? CTxDataLo : CTxArm;
                end

                CTxDataLo: begin
                    q = txToken(dataLo(heldOut.ad, heldOut.parity));
                    tx.advance(False, q, b);
                    state <= CTxDataHi;
                end

                CTxDataHi: begin
                    q = txToken(dataHi(heldOut.ad, heldOut.parity));
                    tx.advance(False, q, b);
                    state <= CTxArm;
                end

                // PTI requires TX_DRIVE to rise only after an idle slot.  The
                // control/data tokens above populate the PLIO-TX retention
                // registers; this explicit idle slot arms the physical drive
                // without clearing those retained values.
                CTxArm: begin
                    q = txToken(ptiToken(PtiIdle, 0, 0));
                    tx.advance(False, q, b);
                    state <= CTxExpose;
                ...
                CTxExpose: begin
                    ...
                    q.responseEnable = heldOut.ack || heldOut.err;
                    q.responseAck = heldOut.ack;
                    q.responseErr = heldOut.err;
