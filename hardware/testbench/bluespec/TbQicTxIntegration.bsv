...
module mkTbQicTxIntegration(Empty);
    ...
    rule stepHarness (!h.ready); h.step(False); endrule
    ...
    rule scenario (h.ready);
        ...
        case (s)
            ...
            1: begin
                ...
                bus<=b; h.start(qic.drivePlio(b,q)); s<=2;
            end
            2: if (h.done) begin
            ...
            14: if (h.done) begin
                BackplaneDrive x=h.backplane;
                if (!x.adParValid || x.ad!=32'h2000 || !x.controlValid || x.control.space!=1 || !x.control.addressStrobe || x.control.read || h.protocolFault) $fatal(1,"D2H address failed");
                $display("QTXTRACE|v1|case=d2h_address|ad=%08x|space=%01x|rd=0|as=1",x.ad,x.control.space);
                b.ack=True; qic.advance(b,q); b.ack=False; bus<=b; s<=15;
            end
            15: begin q.dmaWriteValid=True; q.dmaWrite=DmaWord {data:32'h11223344}; qli<=q; qic.advance(b,q); s<=16; end
            16: begin h.start(qic.drivePlio(b,q)); s<=17; end
            17: if (h.done) begin
                BackplaneDrive x=h.backplane;
                if (!x.adParValid || x.ad!=32'h11223344 || !x.controlValid || !x.control.dataStrobe || h.protocolFault) $fatal(1,"D2H data failed");
                $display("QTXTRACE|v1|case=d2h_data|ad=%08x|par=%01x|ds=1",x.ad,x.par);
                b.ack=True; qic.advance(b,q); b.ack=False; q.dmaWriteValid=False; bus<=b; qli<=q; s<=18;
            end
            18: begin
            ...
            23: begin h.start(qic.drivePlio(b,q)); s<=24; end
            24: if (h.done) begin
                BackplaneDrive x=h.backplane;
                if (!x.adParValid || x.ad!=12 || !x.controlValid || x.control.space!=2 || !x.control.addressStrobe || h.protocolFault) $fatal(1,"notification address failed");
                $display("QTXTRACE|v1|case=notification_address|ad=%08x|space=2|as=1",x.ad);
                b.ack=True; qic.advance(b,q); b.ack=False; bus<=b; s<=25;
            end
            25: begin h.start(qic.drivePlio(b,q)); s<=26; end
            26: if (h.done) begin
                BackplaneDrive x=h.backplane;
                if (!x.adParValid || x.ad!=0 || !x.controlValid || !x.control.dataStrobe || h.protocolFault) $fatal(1,"notification data failed");
