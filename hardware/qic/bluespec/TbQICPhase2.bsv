...
function PlioIn dataWord(Bit#(32) data, Bit#(4) par);
    PlioIn x = plioInDefault();
    x.dataStrobe = True;
    x.adValid = True;
    x.ad = data;
...
function Bit#(2) traceMmioKind(QliIn q);
    if (!q.mmioResponseValid) return 0;
    case (q.mmioResponse.status)
        MmioReadOk: return 1;
        MmioWriteOk: return 2;
        MmioError: return 3;
    endcase
endfunction
...
function String eventName(Bit#(16) c);
    if (c == 0 || c == 14 || c == 274) return "reset";
    if (c == 1 || c == 8 || c == 12 || c == 15) return "worker_address";
    if (c == 13 || c == 273) return "fault";
    return "worker_data";
endfunction
