...
typedef enum {
    ...
    UDmaData,
    UDmaComplete,
    URequestNotification,
    UNotificationAddress,
    UNotificationData
...
module mkPLIOQIC(PLIOQICIfc);
    ...
    Reg#(DmaRequest) dmaReq <- mkReg(DmaRequest {
        direction: HostToDevice,
        address: 0,
        words: BurstOne
    });
    Reg#(DmaCompletion) completion <- mkReg(DmaCompletion {
        status: DmaOk,
        wordsCompleted: 0
    });
    ...
    function Bool validDma(DmaRequest r);
    ...
    function Bool finalReadBuffer();
        return dmaReq.direction == HostToDevice
            && bufferValid
