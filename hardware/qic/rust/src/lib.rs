...
use plio_logical_model::{odd_parity_32, parity_matches, valid_worker_address, BusToCard, BurstWords, CardToBus, Space, PLIO_TIMEOUT_CYCLES};
use qli_model::{DeviceToQic, DmaCompletion, DmaDirection, DmaRequest, DmaStatus, DmaWord, MmioRequest, MmioResponse, NotificationRequest, QicToDevice};
...
enum State {
    ...
    DmaData { request: DmaRequest, completed: u8, wait: u16, buffer: Option<DmaWord> },
    ...
    pub fn drive(&self, bus: &BusToCard, device: &DeviceToQic) -> (CardToBus, QicToDevice) {
        ...
        match self.state {
            State::Idle => {
                if worker_address_cycle(bus) {
                    ...
                    if notification.validate().is_ok() {
                        // Completion-based handshake: asserted only after PLIO ACK.
                    }
            ...
            State::WorkerWriteData { wait, .. } => {
                ...
                if bus.data_strobe {
                    if bus.ad.is_none() || bus.par.is_none() || !parity_matches(bus.ad.unwrap_or(0), bus.par.unwrap_or(0), bus.byte_enable) {
                        card.err = true;
                    }
                }
            }
            ...
            State::NotificationData { request, wait } => {
                ...
                if !timed_out(wait) {
                    ...
                    card.data_strobe = true;
                    if bus.ack { qli.notification_ready = device.notification_request == Some(request); }
                }
            }
        }
        (card, qli)
        ...
        self.state = match self.state {
            State::Idle => {
                if worker_address_cycle(bus) {
                    ...
                    if notification.validate().is_ok() { State::RequestBus(ManagerWork::Notification(notification)) } else { State::Idle }
                } else if let Some(request) = device.dma_request {
                    if request.validate().is_ok() { State::RequestBus(ManagerWork::Dma(request)) } else { State::Idle }
                } else {
                    State::Idle
                }
            }
            State::WorkerWriteData { address, byte_enable, wait } => {
                if timed_out(wait) {
                    ...
                    match (bus.ad, bus.par) {
                        (Some(data), Some(parity)) if parity_matches(data, parity, byte_enable) => State::WorkerOffer {
            ...
            State::WorkerOffer { request, wait } => {
                if timed_out(wait) {
                    ...
                    State::WorkerResponse { read: !request.write, wait: 0 }
                } else {
            ...
            State::DmaAddress(request) => {
                if !bus.grant || bus.err {
                    State::DmaComplete { completion: DmaCompletion { status: DmaStatus::BusError, words_completed: 0 } }
                } else {
            ...
            State::DmaData { request, completed, wait, buffer } => {
                if timed_out(wait) {
                    State::DmaComplete { completion: DmaCompletion { status: DmaStatus::Timeout, words_completed: completed } }
                } else {
                    match request.direction {
                        DmaDirection::HostToDevice => {
                            if let Some(word) = buffer {
                                if device.dma_read_ready {
                                    if completed == request.words.words() {
                                        State::DmaComplete { completion: DmaCompletion { status: DmaStatus::Ok, words_completed: completed } }
                                    } else {
                                        State::DmaData { request, completed, wait: 0, buffer: None }
                                    }
                                } else {
                                    State::DmaData { request, completed, wait, buffer: Some(word) }
                                }
                            } else if bus.err {
                                State::DmaComplete { completion: DmaCompletion { status: DmaStatus::BusError, words_completed: completed } }
                            } else if bus.ack {
                                match (bus.ad, bus.par) {
                                    (Some(data), Some(parity)) if parity_matches(data, parity, 0xf) => {
                                        ...
                                        State::DmaData {
                                            ...
                                            completed: next,
                                            wait: 0,
                                            buffer: Some(DmaWord { data, last: next == request.words.words() }),
                                        }
                                    }
                                    _ => State::DmaComplete { completion: DmaCompletion { status: DmaStatus::ParityError, words_completed: completed } },
                                }
                            } else {
                                State::DmaData { request, completed, wait: wait.saturating_add(1), buffer: None }
                            }
                        }
                        DmaDirection::DeviceToHost => {
                            if let Some(word) = buffer {
                                if bus.err {
                                    ...
                                    if next == request.words.words() {
                                        State::DmaComplete { completion: DmaCompletion { status: DmaStatus::Ok, words_completed: next } }
                                    } else {
                                        State::DmaData { request, completed: next, wait: 0, buffer: None }
                                    }
                                } else {
                                    State::DmaData { request, completed, wait: wait.saturating_add(1), buffer: Some(word) }
                                }
                                ...
                                State::DmaData { request, completed, wait, buffer: None }
                            }
                        }
                    }
                }
            ...
            State::NotificationAddress(request) => {
                if !bus.grant || bus.err { State::Idle } else { State::NotificationData { request, wait: 0 } }
            }
            State::NotificationData { request, wait } => {
