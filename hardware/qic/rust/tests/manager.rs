...
use qli_model::{DeviceToQic, DmaCompletion, DmaDirection, DmaRequest, DmaStatus, DmaWord, NotificationRequest, QicToDevice};
...
fn host_to_device_dma_supports_all_baseline_bursts() {
    for burst in [BurstWords::One, BurstWords::Four, BurstWords::Eight, BurstWords::Sixteen] {
        let mut qic = Qic::new();
        let mut dev = ScriptDevice { dma_request: Some(DmaRequest { direction: DmaDirection::HostToDevice, address: 0x2100_1000, words: burst }), ..ScriptDevice::default() };
...
fn device_to_host_dma_streams_words_and_reports_completion() {
    let burst = BurstWords::Four;
    let words = [0x11, 0x22, 0x33, 0x44];
    let mut dev = ScriptDevice {
        dma_request: Some(DmaRequest { direction: DmaDirection::DeviceToHost, address: 0x3300_2000, words: burst }),
        dma_write_words: words.iter().enumerate().map(|(i, value)| DmaWord { data: *value, last: i + 1 == words.len() }).collect(),
