...
module mkTbPLIOHostDmaM3(Empty);
    ...
    rule run;
        case (phase)
            0: begin dma.bind(2, 3, 32'h2000_0000, 25'h01000, True, True); phase <= 1; end
            1: begin
            ...
            5: begin
                if (beat == 3) begin
                    ...
                    dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 24'h000080), 4, True); beat <= 0; phase <= 6;
                end
            ...
            6: begin
                ...
                dma.memoryRequestAccepted; phase <= 7;
            end
            7: begin dma.memoryResponse(False, True, 32'hb000_0000 | zeroExtend(beat)); phase <= 8; end
            8: begin
                ...
                dma.acknowledgeDeviceRead; phase <= 9;
            end
            9: begin
                if (beat == 3) begin
                    ...
                    dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 1, False); phase <= 10;
                end
                else begin beat <= beat + 1; phase <= 6; end
            end
            10: begin dma.offerDeviceWrite(32'h1122_3344, oddParity32M3(32'h1122_3344)); waits <= 0; phase <= 11; end
            11: begin
                if (waits == 3) begin dma.memoryRequestAccepted; waits <= 0; phase <= 12; end
                else begin dma.waitCycle; waits <= waits + 1; end
            end
            12: begin
                if (waits == 2) begin dma.memoryResponse(False, False, 0); phase <= 13; end
                else begin dma.waitCycle; waits <= waits + 1; end
            end
            13: begin
                ...
                dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 4, False); phase <= 14;
            end
            14: begin dma.offerDeviceWrite(32'h0102_0304, oddParity32M3(32'h0102_0304)); phase <= 15; end
            15: begin dma.memoryRequestAccepted; phase <= 16; end
            16: begin dma.memoryResponse(False, False, 0); phase <= 17; end
            17: begin dma.offerDeviceWrite(32'h5566_7788, 0); phase <= 18; end
            18: begin
                ...
                dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 1, True); phase <= 19;
            end
            19: begin dma.memoryRequestAccepted; phase <= 20; end
            20: begin dma.memoryResponse(True, False, 0); phase <= 21; end
            21: begin
                ...
                dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 1, True); waits <= 0; phase <= 22;
            end
            22: begin
                if (waits == 255) begin dma.waitCycle; phase <= 23; end
                else begin dma.waitCycle; waits <= waits + 1; end
            end
            23: begin
                ...
                dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 1, True); phase <= 24;
            end
            24: begin dma.resetHost; phase <= 25; end
            25: begin
                ...
                dma.clearCompletion; dma.start(2, dmaHandle(3, 0, 0), 4, False); phase <= 26;
            end
            26: begin dma.offerDeviceWrite(32'hdead_beef, oddParity32M3(32'hdead_beef)); phase <= 27; end
            27: begin dma.memoryRequestAccepted; phase <= 28; end
            28: begin dma.revoke(2, 3); phase <= 29; end
            29: begin
                if (dma.capabilityValid(2, 3) || !dma.debugRevokePending) begin $display("FAIL M3 revoke interlock"); $finish(1); end
                dma.memoryResponse(False, False, 0); phase <= 30;
            end
            30: begin
                ...
                dma.clearCompletion; dma.bind(2, 3, 32'h2100_0000, 25'h01000, True, True); phase <= 31;
            end
            31: begin
            ...
            34: begin
                ...
                else begin matrixStep <= matrixStep + 1; phase <= 32; end
            end
        endcase
    endrule
endmodule
