...
rm -rf "$BUILD"
mkdir -p "$BUILD/m1-sim" "$BUILD/m1-rtl" "$BUILD/m2-sim" "$BUILD/m2-rtl" "$BUILD/m3-sim" "$BUILD/m3-rtl"
...
cargo run --quiet --manifest-path "$ROOT/Cargo.toml" -p plio-host-model --bin conformance | tee "$BUILD/m1-rust.log"
...
bsc "${BSC_STACK[@]}" -u -sim -p "$SEARCH" -bdir "$BUILD/m1-sim" -simdir "$BUILD/m1-sim" -info-dir "$BUILD/m1-sim" -g mkTbPLIOWorkerHost "$ROOT/plio-rax-host/bluespec/TbPLIOWorkerHost.bsv"
bsc "${BSC_STACK[@]}" -sim -p "$SEARCH" -bdir "$BUILD/m1-sim" -simdir "$BUILD/m1-sim" -e mkTbPLIOWorkerHost -o "$BUILD/tb-plio-host-m1"
...
cargo run --quiet --manifest-path "$ROOT/Cargo.toml" -p plio-host-manager-model --bin conformance | tee "$BUILD/m2-rust.log"
...
bsc "${BSC_STACK[@]}" -u -sim -p "$SEARCH" -bdir "$BUILD/m2-sim" -simdir "$BUILD/m2-sim" -info-dir "$BUILD/m2-sim" -g mkTbPLIOHostManagerM2 "$ROOT/plio-rax-host/bluespec/TbPLIOHostManagerM2.bsv"
bsc "${BSC_STACK[@]}" -sim -p "$SEARCH" -bdir "$BUILD/m2-sim" -simdir "$BUILD/m2-sim" -e mkTbPLIOHostManagerM2 -o "$BUILD/tb-plio-host-m2"
...
echo "== M3 Rust DMA capability/memory model =="
...
echo "== Generate M1/M2/M3 Verilog =="
bsc "${BSC_STACK[@]}" -u -verilog -p "$SEARCH" -bdir "$BUILD/m1-rtl" -vdir "$BUILD/m1-rtl" -info-dir "$BUILD/m1-rtl" -g mkPLIOWorkerHost "$ROOT/plio-rax-host/bluespec/PLIOWorkerHost.bsv"
bsc "${BSC_STACK[@]}" -u -verilog -p "$SEARCH" -bdir "$BUILD/m2-rtl" -vdir "$BUILD/m2-rtl" -info-dir "$BUILD/m2-rtl" -g mkPLIOHostManagerM2 "$ROOT/plio-rax-host/bluespec/PLIOHostManagerM2.bsv"
bsc "${BSC_STACK[@]}" -u -verilog -p "$SEARCH" -bdir "$BUILD/m3-rtl" -vdir "$BUILD/m3-rtl" -info-dir "$BUILD/m3-rtl" -g mkPLIOHostDmaM3 "$ROOT/plio-rax-host/bluespec/PLIOHostDmaM3.bsv"
test -s "$BUILD/m1-rtl/mkPLIOWorkerHost.v"
test -s "$BUILD/m2-rtl/mkPLIOHostManagerM2.v"
test -s "$BUILD/m3-rtl/mkPLIOHostDmaM3.v"
echo "PASS PLIO host M0/M1/M2/M3 Rust/Bluesim differential and RTL generation"
