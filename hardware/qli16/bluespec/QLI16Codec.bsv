...
module mkQLI16Codec(QLI16CodecIfc);
    ...
    function Bit#(3) qLength(QTxKind k);
        case (k)
            QTxMmioRead: return 2;
            QTxMmioWrite: return 4;
            QTxCancel: return 1;
            QTxDmaRead: return 2;
            QTxDmaCompletion: return 1;
            QTxNotificationCompletion: return 1;
            default: return 0;
        endcase
    endfunction
    ...
    function Bit#(3) dLength(DTxKind k, MmioResponse r);
        case (k)
            ...
            DTxDmaRequest: return 3;
            DTxDmaWrite: return 2;
            DTxNotificationRequest: return 1;
            default: return 0;
        endcase
    endfunction
    ...
    function Qli16Token qToken(QTxKind k, Bit#(3) i);
        ...
        case (k)
            ...
            QTxMmioWrite: begin
                case (i)
                    0: t=mmioHeader0(qMmio);
                    1: t=mmioHeader1(qMmio);
                    2: t=mmioDataLo(0,qMmio.writeData);
                    default: t=mmioDataHi(0,qMmio.writeData);
                endcase
            end
    ...
    function Qli16Token dToken(DTxKind k, Bit#(3) i);
        ...
        case (k)
            DTxMmioReadResponse: begin
                case (i)
                    0: t=mmioResponseStatus(dResponse);
                    1: t=mmioDataLo(1,dResponse.data);
            ...
            DTxDmaRequest: begin
                case (i)
                    0: t=dmaHeader0(dRequest);
                    1: t=dmaHeader1(dRequest);
                    default: t=dmaHeader2(dRequest);
                endcase
            end
    ...
    function Bool qFinalReady(QTxKind k);
        case (k)
            QTxMmioRead, QTxMmioWrite: return devReg.mmioReady;
            QTxCancel: return True;
            QTxDmaRead: return devReg.dmaReadReady;
            QTxDmaCompletion: return devReg.dmaCompletionReady;
            QTxNotificationCompletion: return True;
    ...
    function Bool dFinalReady(DTxKind k);
        case (k)
            DTxMmioReadResponse, DTxMmioSimpleResponse: return qicReg.mmioResponseReady;
            DTxDmaRequest: return qicReg.dmaRequestReady;
            DTxDmaWrite: return qicReg.dmaWriteReady;
            DTxNotificationRequest: return True;
            default: return False;
        endcase
    endfunction
    ...
    function Tuple2#(Bool,Bit#(1)) desiredDirection();
        ...
        if (lastDirValid && lastDir==0 && qActive) return tuple2(True,0);
        if (lastDirValid && lastDir==1 && dActive) return tuple2(True,1);
        if (qActive) return tuple2(True,0);
        if (dActive) return tuple2(True,1);
    ...
    method Action load(QliOut qic, QliIn device) if (!loaded);
        action
            ...
            if (notificationHeldValid) begin
                qi.notificationValid = True;
                qi.notification = notificationHeld;
            end

            if (qic.notificationReady && notificationHeldValid) begin
                notificationHeldValid <= False;
                notificationCompletionPending <= True;
                notificationCompletion <= notificationHeld;
                qi.notificationValid = False;
            end
            ...
            if (dKind == DTxNone) begin
                if (device.mmioResponseValid) begin
                    ...
                    dKind <= (device.mmioResponse.status==MmioReadOk) ? DTxMmioReadResponse : DTxMmioSimpleResponse;
                    dIndex<=0;
                end
    ...
    method Qli16Slot currentSlot if (loaded);
        ...
        if (!hasDir) return idleSlot();
        if (lastDirValid && lastDir != dir) return idleSlot();

        if (dir==0) begin
            Bit#(3) len=qLength(qKind);
            Bool finalToken=(qIndex+1)==len;
    ...
    method Action step if (loaded);
        action
            ...
            if (!hasDir) begin
                ...
                Bit#(3) len=qLength(qKind);
                Bool finalToken=(qIndex+1)==len;
                Bool ack=!finalToken || qFinalReady(qKind);
                if (ack) begin
                    if (!finalToken) qIndex<=qIndex+1;
                        ...
                        case (qKind)
                            ...
                            QTxNotificationCompletion: begin
                                qo.notificationReady=True;
                                notificationCompletionPending<=False;
                            end
                            default: noAction;
                        endcase
                ...
                if (ack) begin
                    if (!finalToken) dIndex<=dIndex+1;
                        ...
                        case (dKind)
                            ...
                            DTxNotificationRequest: begin
                                notificationHeld<=dNotification; notificationHeldValid<=True;
                                qi.notificationValid=True; qi.notification=dNotification;
                            end
                            default: noAction;
                        endcase
            ...
            if (slotsLeft==1) begin slotsLeft<=0; loaded<=False; end
            else slotsLeft<=slotsLeft-1;
        endaction
    endmethod
    ...
    method Action injectRaw(Qli16Token token);
        action
            ...
            case (token.kind)
                ...
                Q16MmioResponse: begin
                    if (token.direction==0) malformed=token.payload!=0;
                    else malformed=(token.payload[15:2]!=0 || token.payload[1:0]==3);
                end
    ...
    method Action resetCodec;
        action
            loaded<=False; slotsLeft<=0; qKind<=QTxNone; dKind<=DTxNone;
            qIndex<=0; dIndex<=0; lastDirValid<=False;
