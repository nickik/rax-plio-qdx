...
fn through_tx(tx: &mut PlioTx, card: CardToBus) -> BackplaneDrive {
    ...
    clock_slot(tx, qdrive(idle()), BackplaneSample::default());
    let q = QicPtiDrive {
        token: idle(), drive_enable: needs_drive,
        response_enable: card.ack || card.err,
        response_ack: card.ack, response_err: card.err,
        bus_request: card.request, ..QicPtiDrive::default()
    };
...
fn sample_ad(tx: &mut PlioTx, ad: u32, par: u8) -> (u32, u8) {
    ...
    let (_, lo) = tx.drive(false, loq, bus); tx.clock(false, loq, bus);
    ...
    let (_, hi) = tx.drive(false, hiq, bus); tx.clock(false, hiq, bus);
...
fn physicalize_bus(tx: &mut PlioTx, mut bus: BusToCard) -> BusToCard {
    if let (Some(ad), Some(par)) = (bus.ad, bus.par) {
        ...
        bus.ad = Some(sampled); bus.par = Some(sampled_par);
    }
    bus
}
...
fn card_from_backplane(bp: BackplaneDrive) -> CardToBus {
    ...
    if let Some((ad, par)) = bp.ad_par { card.ad = Some(ad); card.par = Some(par); }
    if let Some(c) = bp.control {
        card.space = match c.space { 0 => Some(Space::Worker), 1 => Some(Space::HostDma), 2 => Some(Space::Controller), _ => Some(Space::Reserved) };
        card.address_strobe = c.address_strobe; card.read = c.read; card.byte_enable = c.byte_enable;
        card.burst = BurstWords::from_blen(c.burst_len).unwrap(); card.data_strobe = c.data_strobe;
    }
    if let Some((ack, err)) = bp.response { card.ack = ack; card.err = err; }
    card
}

fn worker_cycle(qic: &mut Qic, link: &mut LinkCodec, tx: &mut PlioTx, dev: &mut NakedDevice, peer: &mut TestPeer) {
    ...
    peer.clock(&physical_card); dev.clock(&local.to_device); qic.clock(&bus, &local.to_qic);
}

#[test]
fn naked_worker_read_write_and_error_cross_every_boundary() {
    let mut qic = Qic::new(); let mut link = LinkCodec::new(); let mut tx = PlioTx::new(); let mut dev = NakedDevice::new(); let mut peer = TestPeer::new();
    peer.start_worker_read(CFG_ID, 0xf);
    for _ in 0..64 { worker_cycle(&mut qic,&mut link,&mut tx,&mut dev,&mut peer); if peer.worker_result().is_some(){break;} }
...
fn run_dma(direction: DmaDirection, words: BurstWords, error_beat: Option<u8>, bad_parity_beat: Option<u8>) -> (DmaStatus,u8,Vec<u32>) {
    let mut qic=Qic::new(); let mut link=LinkCodec::new(); let mut tx=PlioTx::new(); let mut peer=TestPeer::new();
    peer.dma_error_beat=error_beat; peer.dma_bad_parity_beat=bad_parity_beat; peer.dma_read_base=0x7000_0000;
    let request=DmaRequest{direction,address:0x1234_5000,words}; let mut request_pending=true; let mut write_index=0u8; let mut received=Vec::new(); let mut final_status=None;
    for _cycle in 0..512 {
        let bus=physicalize_bus(&mut tx,peer.bus_inputs());
        let preview=if request_pending { DeviceToQic{dma_request:Some(request),..DeviceToQic::default()} } else { DeviceToQic::default() };
        let (_,qout)=qic.drive(&bus,&preview);
        let mut device=DeviceToQic::default(); device.dma_completion_ready=true;
        if request_pending { device.dma_request=Some(request); }
        // Do not launch a local DMA word speculatively. QLI-16 is half-duplex:
        // once a word has started it must be held through its final token. Gate
        // each word on the QIC's current semantic readiness so a bus error can
        // immediately turn the link around for the terminal completion.
        if qout.dma_write_ready && !request_pending && direction==DmaDirection::DeviceToHost && write_index<words.words() {
        ...
        let (card,_)=qic.drive(&bus,&local.to_qic); let physical_card=card_from_backplane(through_tx(&mut tx,card));
        peer.clock(&physical_card); qic.clock(&bus,&local.to_qic);
        if final_status.is_some(){break;}
    }
    let (status,completed)=final_status.expect("DMA completion did not cross QLI-16"); (status,completed,received)
}
...
fn both_dma_directions_and_all_burst_sizes_cross_qli16_and_plio_tx() {
    for words in [BurstWords::One,BurstWords::Four,BurstWords::Eight,BurstWords::Sixteen] {
        let (status,completed,_)=run_dma(DmaDirection::DeviceToHost,words,None,None); assert_eq!(status,DmaStatus::Ok); assert_eq!(completed,words.words()); println!("NAKEDTRACE|v1|case=d2h|words={}|status=0",words.words());
        let (status,completed,received)=run_dma(DmaDirection::HostToDevice,words,None,None); assert_eq!(status,DmaStatus::Ok); assert_eq!(completed,words.words()); assert_eq!(received.len(),usize::from(words.words())); println!("NAKEDTRACE|v1|case=h2d|words={}|status=0",words.words());
    }
}

#[test]
fn dma_bus_error_and_bad_parity_preserve_exact_partial_progress() {
    let (status,completed,_)=run_dma(DmaDirection::DeviceToHost,BurstWords::Four,Some(2),None); assert_eq!(status,DmaStatus::BusError); assert_eq!(completed,2);
    let (status,completed,received)=run_dma(DmaDirection::HostToDevice,BurstWords::Four,None,Some(1)); assert_eq!(status,DmaStatus::ParityError); assert_eq!(completed,1); assert_eq!(received.len(),1,"bad parity word must not cross QLI-16");
}

#[test]
fn notification_request_and_completion_cross_same_physical_qli16_link() {
    let mut qic=Qic::new(); let mut link=LinkCodec::new(); let mut tx=PlioTx::new(); let mut peer=TestPeer::new(); let request=NotificationRequest{channel:2}; let mut completed=false; let mut transported=false;
    for _ in 0..128 {
        let bus=physicalize_bus(&mut tx,peer.bus_inputs());
        let device=DeviceToQic{notification_request:Some(request),..DeviceToQic::default()};
        let preview=if transported { DeviceToQic{notification_request:Some(request),..DeviceToQic::default()} } else { DeviceToQic::default() };
        let (_,qout)=qic.drive(&bus,&preview); let local=link.cycle(false,qout,device); transported|=local.to_qic.notification_request==Some(request); completed|=local.to_device.notification_ready;
        let (card,_)=qic.drive(&bus,&local.to_qic); peer.clock(&card_from_backplane(through_tx(&mut tx,card))); qic.clock(&bus,&local.to_qic); if completed{break;}
    }
    assert!(completed,"QLI-16 Notification completion never returned to device"); assert_eq!(peer.notifications(),&[2]); println!("NAKEDTRACE|v1|case=notification|channel=2|ready=1");
}

#[test]
fn reset_and_malformed_qli16_are_local_and_safe() {
    let mut link=LinkCodec::new(); link.inject_raw_token(qli16_model::Token{kind:qli16_model::TokenType::Notification,payload:0x8000,direction:qli16_model::Direction::DeviceToQic}); assert!(link.protocol_fault());
    let reset=link.cycle(true,QicToDevice::default(),DeviceToQic::default()); assert!(!reset.protocol_fault); assert!(reset.to_device.reset);
    let tx=PlioTx::new(); let q=QicPtiDrive{token:idle(),drive_enable:true,bus_request:true,..QicPtiDrive::default()}; let (bp,_)=tx.drive(true,q,BackplaneSample::default()); assert_eq!(bp,BackplaneDrive::default(),"reset must tri-state PLIO-TX");
}
