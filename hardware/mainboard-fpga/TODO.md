...
Current base: `main` already provides the standard `MemoryController` plus the synthesizable 1 MiB `BlockRamBackend`, with `mkDefaultBlockRamBackend` as the system/default FPGA RAM implementation. Mainboard work should consume those modules directly.

## Priority order

### P0 — restore a trustworthy board-level regression

Do not start new mainboard features until these isolated gates are green on the same exact commit.
...
- [x] Replace the overwrite-prone cycle/epoch input boundary with a backpressured registered FIFO boundary.
- [x] CPU `BUS_REQ` → retained `BUS_GRANT` minimal test passes.
- [ ] CPU write → `MemoryController` backend request → completion minimal test passes.
- [ ] CPU read → backend response → CPU `READY/readData` minimal test passes.
- [ ] Reset during an outstanding memory request drops stale response state and recovers.
- [ ] PLIO DMA → shared `MemoryController` path minimal test passes.
- [x] Raw `mkQDXBCard` physical reset-cycle test passes.
- [ ] Mainboard QLIO slot ↔ `mkQDXBCard` reset-cycle test passes on the final board implementation.
- [ ] Full mainboard ↔ physical QDX-B worker transaction passes with explicit PASS/FAIL checking.
- [ ] Full focused mainboard regression passes.
- [ ] Existing memory-controller regressions pass unchanged.
- [ ] Full aggregate Bluespec suite passes.
- [ ] Hardware Rust suite passes.

Debug each failure at the smallest boundary first. Do not change arbitration, memory, and QDX behavior together in one diagnostic patch.
...
This is the first new implementation work after P0 is green.
