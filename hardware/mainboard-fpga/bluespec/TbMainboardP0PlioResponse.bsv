...
module mkTbP0PlioResponse(Empty);
    ...
    rule address0 (stage == P0PlioAddress0);
        ...
        $display("TRACE|p0-plio-response|phase=address0|ack=%0d|err=%0d|role=%0d|plio_req=%0d",
        ...
        if (slotInputs[1].ack || slotInputs[1].err) begin
            $display("FAIL|p0-plio-response|phase=address0");
            $finish(1);
        end
        board.advance(cards, lightingBusMasterDriveDefault(),
            False, noWorkerRequest(),
    ...
    rule address1 (stage == P0PlioAddress1);
        ...
        cards[1] = dmaAddress(32'h3000_0000);
        Vector#(8, PlioIn) slotInputs = board.plioSlots(cards, False);
        $display("TRACE|p0-plio-response|phase=address1|ack=%0d|err=%0d|role=%0d|plio_req=%0d",
            pack(slotInputs[1].ack), pack(slotInputs[1].err),
            pack(board.debugPlioRole), pack(board.debugPlioMemoryRequestValid));
        if (!slotInputs[1].ack || slotInputs[1].err) begin
            $display("FAIL|p0-plio-response|phase=address1");
            $finish(1);
        end
        board.advance(cards, lightingBusMasterDriveDefault(),
            False, noWorkerRequest(),
    ...
    rule readResponse (stage == P0PlioRead);
        ...
        $display("TRACE|p0-plio-response|cycle=%0d|ack=%0d|err=%0d|ad_valid=%0d|ad=%08x|role=%0d|owner=%0d|plio_req=%0d|plio_resp=%0d|cpu_resp=%0d|mc_state=%0d|mc_host_resp=%0d|cycle_pending=%0d|advance_ready=%0d|backend_req=%0d|backend_wr=%0d|backend_addr=%08x|backend_resp_ready=%0d|submit_resp=%0d|response_submitted=%0d",
            cycle, pack(slotInputs[1].ack), pack(slotInputs[1].err),
            pack(slotInputs[1].adValid), slotInputs[1].ad,
            pack(board.debugPlioRole), pack(board.debugMemoryOwner),
            pack(board.debugPlioMemoryRequestValid), pack(board.debugPlioResponsePending),
        ...
        if (slotInputs[1].err) begin
            $display("FAIL|p0-plio-response|slot-error");
            $finish(1);
        end
        ...
        if (slotInputs[1].ack) begin
            ...
            $display("PASS|p0-plio-response|latched memory response reaches PLIO slot");
            $finish(0);
        end
        ...
        cycle <= cycle + 1;
        if (cycle == 16) begin
            $display("FAIL|p0-plio-response|short-watchdog|role=%0d|owner=%0d|plio_resp=%0d|mc_state=%0d|mc_host_resp=%0d|backend_req=%0d|backend_resp_ready=%0d",
                pack(board.debugPlioRole), pack(board.debugMemoryOwner),
                pack(board.debugPlioResponsePending),
