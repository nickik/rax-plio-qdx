...
module mkMainboardFPGA(MainboardFPGAIfc);
    ...
    rule reserveCpuGrant (cycleQ.notEmpty && !cycleQ.first.reset
        && memoryOwner == MainMemNone && !cpuResponsePending && !cpuGrantHeld
        && memory.hostRequestReady && cycleQ.first.cpu.busRequest
        && (!host.memoryRequestValid || preferCpu) && !cycleQ.first.cpu.request);
        cpuGrantHeld <= True;
    endrule

    rule abandonCpuGrant (cycleQ.notEmpty && !cycleQ.first.reset
        && memoryOwner == MainMemNone && !cpuResponsePending && cpuGrantHeld
        && !cycleQ.first.cpu.busRequest && !cycleQ.first.cpu.request);
        cpuGrantHeld <= False;
        preferCpu <= False;
    endrule

    rule rearmCpuRequest (cycleQ.notEmpty && !cycleQ.first.reset
        && memoryOwner == MainMemNone && !cpuResponsePending && cpuRequestSeen
        && !cycleQ.first.cpu.request);
        cpuRequestSeen <= False;
    endrule

    rule retireCpuResponse (cycleQ.notEmpty && !cycleQ.first.reset
        && cpuResponsePending && !cycleQ.first.cpu.request);
        cpuResponsePending <= False;
        cpuResponseFault <= False;
        cpuResponseReadDataValid <= False;
        cpuResponseReadData <= 0;
    ...
    rule startCpuMemoryTransaction (cycleQ.notEmpty && !cycleQ.first.reset
        && memoryOwner == MainMemNone && !cpuResponsePending
        && memory.hostRequestReady && cycleQ.first.cpu.request && !cpuRequestSeen
        && (cpuGrantHeld || (cycleQ.first.cpu.busRequest
            && (!host.memoryRequestValid || preferCpu))));
        let cycle = cycleQ.first;
        memory.hostRequest(cycle.cpu.payload.write, cycle.cpu.payload.addr,
            cycle.cpu.payload.byteEnable, cycle.cpu.payload.writeData);
    ...
    rule captureCpuMemoryResponse (!(cycleQ.notEmpty && cycleQ.first.reset)
        && memoryOwner == MainMemCpu && memory.hostResponseValid);
    ...
    rule advancePlioRequest (cycleQ.notEmpty && !cycleQ.first.reset
        && !cpuResponsePending && memoryOwner == MainMemNone && !cpuGrantHeld
        && memory.hostRequestReady && host.memoryRequestValid
        && (!cycleQ.first.cpu.busRequest || !preferCpu));
        let cycle = cycleQ.first;
        Vector#(8, PlioOut) logicalCards = plioCardsFromBackplane(cycle.cards);
        host.advance(logicalCards, cycle.workerValid, cycle.workerRequest,
            True, False, False, False, 0, False);
        memory.hostRequest(host.memoryWrite, host.memoryAddress, 4'hf,
    ...
    rule advancePlioResponse (cycleQ.notEmpty && !cycleQ.first.reset
        ...
        Vector#(8, PlioOut) logicalCards = plioCardsFromBackplane(cycle.cards);
        host.advance(logicalCards, cycle.workerValid, cycle.workerRequest,
            False, True, memory.hostResponseFault, memory.hostReadDataValid,
            memory.hostReadData, False);
        memory.hostResponseConsumed;
        memoryOwner <= MainMemNone;
        preferCpu <= True;
        cycleQ.deq;
    ...
    method LightingBusInputs lightingMemory(Vector#(8, BackplaneDrive) cards,
        ...
        if (!reset) begin
            ...
            Bool canArbitrate = memoryOwner == MainMemNone && !cpuResponsePending
                && !cpuGrantHeld && memory.hostRequestReady;
            Bool selectCpu = canArbitrate && cpu.busRequest
                && (!plioWaiting || preferCpu);
            Bool cpuOwnsBus = cpuGrantHeld || selectCpu
                || memoryOwner == MainMemCpu || cpuResponsePending;
            if (cpuOwnsBus) begin
                out.busGrant = True;
    ...
    method LightingModuleInterrupts interrupts(Bool timerIrq, Bool machineFault);
        return LightingModuleInterrupts { plioIrq: host.claimValid,
            timerIrq: timerIrq, machineFault: machineFault };
    endmethod
    ...
    method Action advance(Vector#(8, BackplaneDrive) cards,
        LightingBusMasterDrive cpu, Bool workerValid, HostWorkerRequest workerRequest,
        Bool backendRequestReady, Bool backendResponseValid, Bool backendFault,
        Bool backendReadDataValid, Bit#(32) backendReadData, Bool reset)
        if (cycleQ.notFull);
        cycleQ.enq(MainboardCycleInputs { cards: cards, cpu: cpu,
            workerValid: workerValid, workerRequest: workerRequest,
            backendRequestReady: backendRequestReady,
            backendResponseValid: backendResponseValid, backendFault: backendFault,
            backendReadDataValid: backendReadDataValid,
            backendReadData: backendReadData, reset: reset });
    endmethod
    ...
    endmethod
    method Action revokeDma(Bit#(3) slot, Bit#(4) channel); host.revokeDma(slot, channel); endmethod
    method Bit#(4) dmaGeneration(Bit#(3) slot, Bit#(4) channel) = host.dmaGeneration(slot, channel);
    ...
    method Action clearWorkerCompletion; host.clearWorkerCompletion; endmethod
    ...
    method Action clearDmaCompletion; host.clearDmaCompletion; endmethod
    method Action setNotificationConfig(Bit#(3) slot, Bit#(2) channel,
        Bool enabled, Bool masked, Bit#(4) classCode);
    ...
    method Action claimFirst; host.claimFirst; endmethod
    ...
    method Bool debugPlioResponsePending = memoryOwner == MainMemPlio && memory.hostResponseValid;
