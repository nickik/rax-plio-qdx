...
module mkTbMainboardM6CpuFaultIsolation(Empty);
    ...
    rule r4(s==4&&b.debugAdvanceReady); Vector#(8,BackplaneDrive)c=card1(dmaAddress(32'h30000000,True)); Vector#(8,PlioIn)i=b.plioSlots(c,False);if(!i[1].ack||i[1].err) begin $display("FAIL|m6.2|cpu-isolation-address");$finish(1);end b.advance(c,lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=5;endrule
...
module mkTbMainboardM6CpuWriteFault(Empty);
    ...
    rule r5(s==5&&b.debugAdvanceReady&&b.debugCpuResponsePending);LightingBusInputs x=b.lightingMemory(idleCards(),cpuReq(32'h1800,True),False);Vector#(8,PlioIn)i=b.plioSlots(idleCards(),False);if(!x.ready||!x.error||i[1].ack||i[1].err) begin $display("FAIL|m6.2|cpu-write-routing");$finish(1);end b.advance(idleCards(),cpuReq(32'h1800,True),False,noWorkerRequest(),False,False,False,False,0,False);$display("M6FAULT|owner=cpu|op=write|cpu_fault=1|plio_spurious=0|status=ok");$display("PASS|m6.2-cpu-write|CPU write fault routed exactly to CPU");$finish(0);endrule
endmodule
...
module mkPlioFault#(Bool isRead)(Empty);
    ...
    rule r4(s==4&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=card1(dmaAddress(32'h30000000,isRead));Vector#(8,PlioIn)i=b.plioSlots(c,False);if(!i[1].ack||i[1].err) begin $display("FAIL|m6.2|plio-address|read=%0d",pack(isRead));$finish(1);end b.advance(beat(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=5;endrule
    rule waitForPlioRequest(s==5&&b.debugAdvanceReady&&!b.memoryBackendRequestValid);b.advance(beat(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);endrule
