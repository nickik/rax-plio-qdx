...
function Vector#(8,BackplaneDrive) card1(BackplaneDrive d);Vector#(8,BackplaneDrive)c=idleCards();c[1]=d;return c;endfunction
...
module mkTbMainboardM6ResetCpuOwner(Empty);
 ...
 rule r4(s==4&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=card1(dmaAddress(32'h30000000));Vector#(8,PlioIn)i=b.plioSlots(c,False);if(!i[1].ack||i[1].err) begin $display("FAIL|m6.3|cpu-owner-address");$finish(1);end b.advance(c,lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=5;endrule
 ...
 rule stale(s==10&&b.debugAdvanceReady);if(b.debugMemoryOwner!=MainMemNone||b.memoryBackendRequestValid||b.memoryBackendResponseReady||b.debugPlioMemoryRequestValid||b.debugPlioFaultValid) begin $display("FAIL|m6.3|cpu-owner-stale|owner=%0d|backend_req=%0d|backend_ready=%0d|plio_pending=%0d|plio_fault=%0d",pack(b.debugMemoryOwner),pack(b.memoryBackendRequestValid),pack(b.memoryBackendResponseReady),pack(b.debugPlioMemoryRequestValid),pack(b.debugPlioFaultValid));$finish(1);end $display("M6RESET|owner=cpu|waiting=plio|cleared=1|stale_suppressed=1|status=ok");s<=11;endrule
 // Reset abandons transfers but preserves DMA capabilities; rebinding would
 // deliberately advance the generation and invalidate the recovery handle.
 rule rb(s==11);s<=12;endrule
 ...
 rule a1(s==14&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=card1(dmaAddress(32'h30000000));Vector#(8,PlioIn)i=b.plioSlots(c,False);if(!i[1].ack||i[1].err) begin $display("FAIL|m6.3|cpu-owner-recovery-address|ack=%0d|err=%0d|role=%0d|dma=%0d|fault_valid=%0d|fault=%0d",pack(i[1].ack),pack(i[1].err),pack(b.debugPlioRole),pack(b.debugPlioDmaState),pack(b.debugPlioFaultValid),pack(b.debugPlioFault));$finish(1);end b.advance(pc(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=15;endrule
 rule waitPlioRecovery(s==15&&b.debugAdvanceReady&&!b.memoryBackendRequestValid);b.advance(pc(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);endrule
...
module mkTbMainboardM6ResetPlioOwner(Empty);
 ...
 rule r2(s==2&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=idleCards();c[1]=requestOnly();b.advance(c,lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=3;endrule
 rule r3(s==3&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=idleCards();c[1]=dmaAddress(32'h30000000);b.advance(c,lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=4;endrule
 rule r4(s==4&&b.debugAdvanceReady);Vector#(8,BackplaneDrive)c=card1(dmaAddress(32'h30000000));Vector#(8,PlioIn)i=b.plioSlots(c,False);if(!i[1].ack||i[1].err) begin $display("FAIL|m6.3|plio-owner-address");$finish(1);end b.advance(pc(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);s<=5;endrule
 rule waitPlioOwner(s==5&&b.debugAdvanceReady&&!b.memoryBackendRequestValid);b.advance(pc(),lightingBusMasterDriveDefault(),False,noWorkerRequest(),False,False,False,False,0,False);endrule
 rule own(s==5&&b.debugAdvanceReady&&b.memoryBackendRequestValid);LightingBusInputs x=b.lightingMemory(pc(),cpuBR(),False);if(b.debugMemoryOwner!=MainMemPlio) begin $display("FAIL|m6.3|plio-owner-pre");$finish(1);end if(x.busGrant) begin $display("FAIL|m6.3|plio-owner-cpu-grant");$finish(1);end b.advance(pc(),cpuBR(),False,noWorkerRequest(),True,False,False,False,0,False);s<=6;endrule
 ...
 rule stale(s==8&&b.debugAdvanceReady);if(b.debugMemoryOwner!=MainMemNone||b.memoryBackendRequestValid||b.memoryBackendResponseReady||b.debugPlioMemoryRequestValid||b.debugPlioFaultValid) begin $display("FAIL|m6.3|plio-owner-stale|owner=%0d|backend_req=%0d|backend_ready=%0d|plio_pending=%0d|plio_fault=%0d",pack(b.debugMemoryOwner),pack(b.memoryBackendRequestValid),pack(b.memoryBackendResponseReady),pack(b.debugPlioMemoryRequestValid),pack(b.debugPlioFaultValid));$finish(1);end $display("M6RESET|owner=plio|waiting=cpu|cleared=1|stale_suppressed=1|status=ok");s<=9;endrule
 ...
 rule ro(s==13&&b.debugAdvanceReady&&b.debugCpuResponsePending);LightingBusInputs x=b.lightingMemory(idleCards(),cpuRead(ca),False);if(!x.ready||x.error||x.readData!=cd) begin $display("FAIL|m6.3|plio-owner-recovery-data");$finish(1);end b.advance(idleCards(),cpuRead(ca),False,noWorkerRequest(),False,False,False,False,0,False);$display("PASS|m6.3-plio-owner|reset clears PLIO owner and waiting CPU, suppresses stale completion, fresh CPU recovers");$finish(0);endrule
endmodule
endpackage
