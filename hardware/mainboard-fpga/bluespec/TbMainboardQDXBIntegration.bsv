...
module mkTbMainboardQDXBIntegration(Empty);
    ...
    rule observeProgress (stage != QbDone);
        ...
        if (clockTicks != 0 && clockTicks[9:0] == 0) begin
            $display("DBG|qdx-b|clock=%0d|physical_cycles=%0d|stage=%0d|launched=%0d|worker_sent=%0d|role=%0d|qic=%0d|qdx=%0d|owner=%0d|plio_mem_req=%0d|host_fault_valid=%0d|host_fault=%0d|card_fault=%0d",
        ...
        if (clockTicks == 32'd200000) begin
            $display("FAIL|qdx-b|clock-watchdog|physical_cycles=%0d|stage=%0d|launched=%0d|worker_sent=%0d|role=%0d|qic=%0d|qdx=%0d|owner=%0d|plio_mem_req=%0d|host_fault_valid=%0d|host_fault=%0d|card_fault=%0d",
    ...
    rule consumeCardCycle (launched && card.cycleDone);
        ...
        if (watchdog[5:0] == 0) begin
            $display("DBG|qdx-b-cycle|physical_cycles=%0d|stage=%0d|reset=%0d|send_worker=%0d|request=%0d|control_valid=%0d|response_valid=%0d|role=%0d|qic=%0d|qdx=%0d|owner=%0d|plio_mem_req=%0d|host_fault_valid=%0d|card_fault=%0d",
                ...
                pack(card.qdxState), pack(board.debugMemoryOwner),
                pack(board.debugPlioMemoryRequestValid),
                pack(board.debugPlioFaultValid), pack(card.protocolFault));
        end
    ...
    rule checkWorkerCompletion (
        ...
        if (card.protocolFault || card.qdxError != QdxErrNone) begin
            $display("FAIL|qdx-b|final-card-state|card_fault=%0d|qdx_error=%0d",
