...
module mkQDXBEndpoint#(QDXBMediaIfc media)(QDXBEndpointIfc);
 ...
 method Action advance(QdxAEndpointOut qdx, QdxAProfileDmaOut dma); action
  if (qdx.reset) begin state<=BIdle; dmaPhase<=BDmaIdle; dmaKind<=BDmaNone; lastStatus<=0; sgEntry<=0; sgPart<=0; segmentIndex<=0; segmentWordOffset<=0; transferIndex<=0; transferWords<=0; end
  ...
  else begin case (dmaPhase)
   BDmaRequest: if (dma.requestReady) begin dmaMoved<=0; dmaPhase<=BDmaTransfer; end
   BDmaTransfer: begin if (dmaReq.direction==HostToDevice && dma.readValid) begin if (dmaKind==BDmaSgAddress) begin Vector#(16,QdxBSgEntry) v=sg; QdxBSgEntry e=v[truncate(sgEntry)]; e.address=dma.readWord.data; v[truncate(sgEntry)]=e; sg<=v; end else if (dmaKind==BDmaSgLength) begin Vector#(16,QdxBSgEntry) v=sg; QdxBSgEntry e=v[truncate(sgEntry)]; e.lengthBytes=dma.readWord.data; v[truncate(sgEntry)]=e; sg<=v; end else if (dmaKind==BDmaPayload) stage.upd(truncate(transferIndex+zeroExtend(dmaMoved)),dma.readWord.data); Bit#(5) n=dmaMoved+1; dmaMoved<=n; if (n==burstCount(dmaReq.words)) dmaPhase<=BDmaCompletion; end else if (dmaReq.direction==DeviceToHost && dma.writeReady) begin Bit#(5) n=dmaMoved+1; dmaMoved<=n; if (n==burstCount(dmaReq.words)) dmaPhase<=BDmaCompletion; end end
   ...
   if (dmaPhase==BDmaIdle) begin if (state==BFetchSg) begin if (sgEntry>=zeroExtend(cmd.sgCount)) begin Vector#(16,QdxBSgEntry) currentSg=sg; if (!sgValid(currentSg,cmd,opKind)) begin completion<=makeCompletion(cmd,stInvalidField,0,0); lastStatus<=stInvalidField; state<=BComplete; end else begin transferIndex<=0; segmentIndex<=0; segmentWordOffset<=0; state<=BTransfer; end end else begin Bit#(32) delta=(zeroExtend(sgEntry)<<3)+(zeroExtend(sgPart)<<2); Bit#(32) a=handleAdd(cmd.sgAddr,delta); dmaReq<=DmaRequest {direction:HostToDevice,address:a,words:BurstOne}; dmaKind<=sgPart==0 ? BDmaSgAddress : BDmaSgLength; dmaMoved<=0; dmaPhase<=BDmaRequest; end end
    else if (state==BTransfer) begin if (transferIndex>=transferWords) begin if (opKind==BOpWrite || opKind==BOpWriteDurable) begin commitIndex<=0; state<=BMediaCommit; end else begin Bit#(32) blocks=(opKind==BOpRead)?1:0; completion<=makeCompletion(cmd,stSuccess,0,blocks); lastStatus<=stSuccess; state<=BComplete; end end else if (cmd.sgCount==0) begin Bit#(16) remain=transferWords-transferIndex; BurstWords b=chooseBurst(remain); Bit#(32) a=handleAdd(cmd.dataAddr,zeroExtend(transferIndex)<<2); dmaReq<=DmaRequest {direction:(opKind==BOpWrite || opKind==BOpWriteDurable)?HostToDevice:DeviceToHost,address:a,words:b}; dmaKind<=BDmaPayload; dmaMoved<=0; dmaPhase<=BDmaRequest; end else if (segmentIndex>=zeroExtend(cmd.sgCount)) begin completion<=makeCompletion(cmd,stInvalidField,0,0); lastStatus<=stInvalidField; state<=BComplete; end else begin Vector#(16,QdxBSgEntry) currentSg=sg; QdxBSgEntry e=currentSg[truncate(segmentIndex)]; Bit#(16) segWords=truncate(e.lengthBytes>>2); if (segmentWordOffset>=segWords) begin segmentIndex<=segmentIndex+1; segmentWordOffset<=0; end else begin Bit#(16) remain=transferWords-transferIndex; Bit#(16) segRemain=segWords-segmentWordOffset; Bit#(16) possible=(remain<segRemain)?remain:segRemain; BurstWords b=chooseBurst(possible); Bit#(32) a=handleAdd(e.address,zeroExtend(segmentWordOffset)<<2); dmaReq<=DmaRequest {direction:(opKind==BOpWrite || opKind==BOpWriteDurable)?HostToDevice:DeviceToHost,address:a,words:b}; dmaKind<=BDmaPayload; dmaMoved<=0; dmaPhase<=BDmaRequest; end end end
    else if (state==BMediaCommit) begin Bit#(32) staged=stage.sub(commitIndex); media.writeWord(cmd.namespaceId,cmd.lba,commitIndex,staged); if (zeroExtend(commitIndex)+1>=transferWords) begin Bit#(16) f=(opKind==BOpWriteDurable)?cfWriteDurableDone:0; completion<=makeCompletion(cmd,stSuccess,f,1); lastStatus<=stSuccess; state<=BComplete; end else commitIndex<=commitIndex+1; end end end
 endaction endmethod
